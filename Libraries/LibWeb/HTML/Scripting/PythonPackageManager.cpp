...
String PythonPackageManager::get_python_home_path() const
{
    ...
    if (bundle_path.has_value()) {
        auto python_home_result = String::formatted(
            "{}/Contents/Resources/bundled_python/Versions/3.14", bundle_path.value());
    ...
    return String("/opt/homebrew/opt/python@3.14/Frameworks/Python.framework/Versions/3.14"_string);
}
...
String PythonPackageManager::get_package_install_path() const
{
    ...
    // Fallback
    auto fallback_result = String::formatted("{}/lib/python3.14/site-packages", python_home);
...
ErrorOr<void> PythonPackageManager::initialize()
{
    ...
    if (stat(site_packages_byte_string.characters(), &buffer) != 0) {
        dbgln("🐍 PythonPackageManager: Warning - site-packages not found at {}", site_packages);
    ...
    URL::URL const& document_origin)
{
    ...
    while (!lexer.is_eof()) {
        ...
        while (!lexer.is_eof() && lexer.peek() != '\n' && lexer.peek() != '\r' && lexer.peek() != '#' && !(lexer.peek() == '=' && lexer.peek(1) == '=') && !(lexer.peek() == '>' && lexer.peek(1) == '=') && !(lexer.peek() == '<' && lexer.peek(1) == '=') && !(lexer.peek() == '~' && lexer.peek(1) == '=') && lexer.peek() != '>' && lexer.peek() != '<') {
    ...
    for (auto const& package : packages_to_install) {
        ...
        if (package.version.has_value()) {
            ...
            if (!trimmed_version.is_error() && !trimmed_version.value().is_empty()) {
                ...
                if (!trimmed_version.value().starts_with_bytes("="sv) && !trimmed_version.value().starts_with_bytes(">"sv) && !trimmed_version.value().starts_with_bytes("<"sv) && !trimmed_version.value().starts_with_bytes("~"sv)) {
