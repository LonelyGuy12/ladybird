...
if (NOT WIN32)
    ...
    # Force Clang on macOS for compatibility with Objective-C/Objective-C++ code (ANGLE, ffmpeg, etc.)
    if (APPLE)
        if ("${CMAKE_C_COMPILER}" STREQUAL "" OR NOT "${CMAKE_C_COMPILER}" MATCHES "clang")
            string(APPEND EXTRA_VCPKG_VARIABLES "set(ENV{CC} /usr/bin/clang)\n")
            string(APPEND EXTRA_VCPKG_VARIABLES "set(CMAKE_C_COMPILER /usr/bin/clang CACHE FILEPATH \"\" FORCE)\n")
        else()
            string(APPEND EXTRA_VCPKG_VARIABLES "set(ENV{CC} ${CMAKE_C_COMPILER})\n")
        endif()
        if ("${CMAKE_CXX_COMPILER}" STREQUAL "" OR NOT "${CMAKE_CXX_COMPILER}" MATCHES "clang")
            string(APPEND EXTRA_VCPKG_VARIABLES "set(ENV{CXX} /usr/bin/clang++)\n")
            string(APPEND EXTRA_VCPKG_VARIABLES "set(CMAKE_CXX_COMPILER /usr/bin/clang++ CACHE FILEPATH \"\" FORCE)\n")
        else()
