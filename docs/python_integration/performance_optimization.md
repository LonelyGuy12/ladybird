...
Performance is critical for Python integration in the Ladybird browser. Python scripts must execute efficiently to provide a competitive alternative to JavaScript while maintaining the ease of use and familiarity that Python developers expect.

## Performance Goals

1. **Execution Speed**: Python script execution should be within 2x of equivalent JavaScript performance
2. **Memory Efficiency**: Memory usage should be comparable to JavaScript
3. **Startup Time**: Python engine initialization should be sub-second
4. **Responsiveness**: UI should remain responsive during Python script execution
5. **Scalability**: Performance should scale well with increasing script complexity

## Current Performance Profile

### Baseline Measurements

Initial performance measurements show:
- Python script parsing: ~5-10ms for typical scripts
- Python script execution: ~10-50ms for typical scripts
- DOM API access: ~1-5ms per call
- Cross-language bridges: ~5-15ms overhead per call

### Comparison with JavaScript

JavaScript baseline performance:
- JavaScript script parsing: ~2-5ms for typical scripts
- JavaScript script execution: ~5-25ms for typical scripts
- DOM API access: ~0.5-2ms per call
- No cross-language bridge overhead (native)
...
Cache frequently accessed methods to avoid repeated lookups:
...
#### Bytecode Caching
Cache compiled Python bytecode to avoid recompilation:

```cpp
// PythonEngine.cpp
static HashMap<String, PyObject*> s_bytecode_cache;

PyObject* PythonEngine::get_cached_bytecode(const String& source, const String& filename) {
    auto cache_key = String::formatted("{}:{}", filename, source);
    if (auto it = s_bytecode_cache.find(cache_key); it != s_bytecode_cache.end()) {
        return it->value;
    }
    
    // Compile and cache
    PyObject* compiled_code = Py_CompileString(source.characters(), filename.characters(), Py_file_input);
    if (compiled_code) {
        s_bytecode_cache.set(cache_key, compiled_code);
...
Minimize copying for string values:
